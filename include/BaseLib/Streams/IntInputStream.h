...
#ifndef BaseLib_Streams_IntInputStream_h
#define BaseLib_Streams_IntInputStream_h
...
/**
Converts every read call to a read(int) call.
Beside an InputStream the stream can be connected to a functor alternatively.
Read-calls of every type get converted into read(int) calls.
@see @ref BuffersAndStreams, IntOutputStream, InputStream, connectToFunctor()
*/
class BL_EXPORT IntInputStream : public InputStream {
public:
	...
	IntInputStream();
	explicit IntInputStream(const InputStream& connectToInputStream);
	explicit IntInputStream(const Functor& connectToGetterFunctor);
	explicit IntInputStream(const int* value);
	explicit IntInputStream(int value);
...
#endif // #ifndef BaseLib_Streams_IntInputStream_h
