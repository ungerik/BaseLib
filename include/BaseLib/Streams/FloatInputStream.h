...
#ifndef BaseLib_Streams_FloatInputStream_h
#define BaseLib_Streams_FloatInputStream_h
...
/**
Converts every read call to a read(float) call.
Beside an InputStream the stream can be connected to a functor alternatively.
Read-calls of every type get converted into read(float) calls.
@see @ref BuffersAndStreams, FloatOutputStream, InputStream, connectToFunctor()
*/
class BL_EXPORT FloatInputStream : public InputStream {
public:
	...
	FloatInputStream();
	explicit FloatInputStream(const InputStream& connectToInputStream);
	explicit FloatInputStream(const Functor& connectToGetterFunctor);
	explicit FloatInputStream(const float* value);
	explicit FloatInputStream(float value);
...
#endif // #ifndef BaseLib_Streams_FloatInputStream_h
