...
struct BL_ALIGN_SIMD(VectorPad3DT) {
	...
	inline explicit VectorPad3DT<T>(const T sourceArray[3])
		: x(sourceArray[0])
		, y(sourceArray[1])
		, z(sourceArray[2])
	{
	...
	inline VectorPad3DT<T>& operator -()
	{
		assertInitialized();
		x = -x;
		y = -y;
...
typedef VectorPad3DT<double> VectorPad3Dd;
	...
	typedef VectorPad3Dd VectorPad3D;
#else
	typedef VectorPad3Df VectorPad3D;
#endif
...
namespace BaseLib {
	namespace TypeTraits {
		...
		BL_DEFINE_ISVALUETYPE(::BaseLib::Math::VectorPad3Df)
		BL_DEFINE_ISVALUETYPE(::BaseLib::Math::VectorPad3Dd)
