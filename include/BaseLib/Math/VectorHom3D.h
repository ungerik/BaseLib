...
struct BL_ALIGN_SIMD(VectorHom3DT) {
	...
	inline explicit VectorHom3DT<T>(const T sourceArray[4])
		: x(sourceArray[0])
		, y(sourceArray[1])
		, z(sourceArray[2])
		, w(sourceArray[3])
	...
	inline VectorHom3DT<T>& operator -()
	{
		assertInitialized();
		x = -x;
		y = -y;
...
typedef VectorHom3DT<float> VectorHom3Df;
typedef VectorHom3DT<double> VectorHom3Dd;

#ifdef BL_MATH_DOUBLE_PRECISION
	typedef VectorHom3Dd VectorHom3D;
#else
	typedef VectorHom3Df VectorHom3D;
#endif
...
namespace BaseLib {
	namespace TypeTraits {
		...
		BL_DEFINE_ISVALUETYPE(::BaseLib::Math::VectorHom3Df)
		BL_DEFINE_ISVALUETYPE(::BaseLib::Math::VectorHom3Dd)
