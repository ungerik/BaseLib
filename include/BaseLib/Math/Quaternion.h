...
struct BL_ALIGN_SIMD(QuaternionT) {
	...
	inline explicit QuaternionT<T>(const T sourceArray[4])
		: x(sourceArray[0])
		, y(sourceArray[1])
		, z(sourceArray[2])
		, w(sourceArray[3])
	...
	inline QuaternionT<T>& operator -() // Should that be unary operator -, or should inverse be -??
	{
		x = -x;
		y = -y;
		z = -z;
...
namespace BaseLib {
	namespace TypeTraits {
		...
		BL_DEFINE_ISVALUETYPE(::BaseLib::Math::Quaternionf)
		BL_DEFINE_ISVALUETYPE(::BaseLib::Math::Quaterniond)
