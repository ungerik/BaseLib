...
template<typename T> struct UnitVector3DT;
...
struct Vector3DT {
	...
	inline explicit Vector3DT<T>(const T sourceValues[3])
		: x(sourceValues[0])
		, y(sourceValues[1])
		, z(sourceValues[2])
	{
	...
	inline Vector3DT<T>(const UnitVector3DT<T>& unitVector)
	{
		operator =(unitVector);
	}
	...
	inline Vector3DT<T>& operator =(const UnitVector3DT<T>& right)
	{
		x = right.x;
		y = right.y;
		z = right.z;
	...
	inline UnitVector3DT<T> getUnitVector()
	{
		return UnitVector3DT<T>(clone().normalize());
	}
...
typedef Vector3DT<double> Vector3Dd;
	...
	typedef Vector3Dd Vector3D;
#else
	typedef Vector3Df Vector3D;
#endif
...
template<typename T>
struct UnitVector3DT : Vector3DT<T> {

	inline UnitVector3DT<T>()
		: Vector3DT<T>()
	{
	}

	inline UnitVector3DT<T>(T newX, T newY, T newZ)
		: Vector3DT<T>(newX, newY, newZ)
	{
		assertNormalized();
	}

	inline explicit UnitVector3DT<T>(const T sourceValues[3])
		: Vector3DT<T>(sourceValues)
	{
		assertNormalized();
	}

	inline explicit UnitVector3DT<T>(const UnitVector3DT<T>& other)
		: Vector3DT<T>(other)
	{
	}

	inline explicit UnitVector3DT<T>(const Vector3DT<T>& vector3D)
		: Vector3DT<T>(vector3D)
	{
		assertNormalized();
	}

	inline UnitVector3DT<T> operator =(const UnitVector3DT<T>& right)
	{
		return Vector3DT<T>::operator =(right);
	}

	inline void assertNormalized() const
	{
		#ifdef BL_MATH_RANGE_CHECK
			BL_ASSERT(isUnitLength());
		#endif
	}

	inline UnitVector3DT<T>& setZero()
	{
		x = 0;
		y = 0;
		z = 0;
	...
	inline UnitVector3DT<T>& setUnitX()
	{
		x = 1;
		y = 0;
		z = 0;
	...
	inline UnitVector3DT<T>& setUnitY()
	{
		x = 0;
		y = 1;
		z = 0;
	...
	inline UnitVector3DT<T>& setUnitZ()
	{
		x = 0;
		y = 0;
		z = 1;
	...
	inline UnitVector3DT<T>(Zero)
	{
		setZero();
	}

	inline UnitVector3DT<T>& operator =(Zero)
	{
		return setZero();
	}

	inline UnitVector3DT<T>(UnitX)
	{
		setUnitX();
	}

	inline UnitVector3DT<T>& operator =(UnitX)
	{
		return setUnitX();
	}

	inline UnitVector3DT<T>(UnitY)
	{
		setUnitY();
	}

	inline UnitVector3DT<T>& operator =(UnitY)
	{
		return setUnitY();
	}

	inline UnitVector3DT<T>(UnitZ)
	{
		setUnitZ();
	}

	inline UnitVector3DT<T>& operator =(UnitZ)
	{
		return setUnitZ();
	}
...
typedef UnitVector3DT<float> UnitVector3Df;
typedef UnitVector3DT<double> UnitVector3Dd;

#ifdef BL_MATH_DOUBLE_PRECISION
	typedef UnitVector3Dd UnitVector3D;
#else
	typedef UnitVector3Df UnitVector3D;
#endif
...
namespace BaseLib {
	namespace TypeTraits {
		...
		BL_DEFINE_ISVALUETYPE(::BaseLib::Math::Vector3Df)
		BL_DEFINE_ISVALUETYPE(::BaseLib::Math::Vector3Dd)
